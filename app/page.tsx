...
export default function HomePage() {
  return (
    <>
      {/* Hero */}
      <section className="bg-gradient-to-br from-[#2563EB] to-[#1e40af] text-white py-16 px-4">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-10">
            ...
            <Link
              ...
              className="inline-flex items-center justify-center gap-2 bg-white/20 hover:bg-white/30 text-white font-bold text-base px-8 py-4 rounded-full transition-colors no-underline border border-white/40"
            >
              工法の違いを見る
            </Link>
          </div>
          <div className="grid grid-cols-3 gap-4 max-w-lg mx-auto">
            <div className="text-center">
              <div className="text-2xl font-bold text-[#F97316]">300社+</div>
              <div className="text-xs text-blue-200">加盟業者数</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-[#F97316]">無料</div>
              <div className="text-xs text-blue-200">見積もり費用</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-[#F97316]">10年</div>
      ...
      <section className="max-w-6xl mx-auto px-4 py-12">
        <div className="text-center mb-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">防水工法から探す</h2>
          <p className="text-gray-600">あなたのベランダに最適な工法を見つけましょう</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {methodsData.slice(0, 3).map((method) => (
            <Link
              ...
              className="bg-white rounded-xl p-6 border border-gray-200 hover:border-[#2563EB] hover:shadow-md transition-all no-underline group"
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-xs font-bold text-[#059669] bg-green-50 px-2 py-1 rounded-full">
                  {method.costPerSqm}
                </span>
              </div>
              <h3 className="font-bold text-gray-900 mb-2">{method.title}</h3>
              <p className="text-sm text-gray-500 mb-3">{method.description.slice(0, 60)}...</p>
              <div className="flex items-center justify-between text-xs text-gray-500">
                <span>耐用年数：{method.durability}</span>
                <span className="text-[#2563EB] font-medium">詳しく見る →</span>
              </div>
            </Link>
          ))}
          <Link
            href="/method/comparison/"
            className="bg-white rounded-xl p-6 border border-dashed border-[#2563EB] hover:bg-[#EFF6FF] transition-all no-underline flex flex-col items-center justify-center text-center"
          >
          ...
          <Link
            href="/method/topcoat/"
            className="bg-white rounded-xl p-6 border border-gray-200 hover:border-[#059669] hover:shadow-md transition-all no-underline"
          >
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-xs font-bold text-[#059669] bg-green-50 px-2 py-1 rounded-full">最安メンテ</span>
            </div>
            <h3 className="font-bold text-gray-900 mb-2">トップコート塗り替え</h3>
            <p className="text-sm text-gray-500 mb-3">防水層を長持ちさせる延命メンテナンス</p>
            <div className="flex items-center justify-between text-xs text-gray-500">
              <span>1,000〜2,500円/㎡</span>
              <span className="text-[#059669] font-medium">詳しく見る →</span>
            </div>
          </Link>
        </div>
      </section>
      ...
      <section className="bg-white py-12 px-4">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 bg-orange-50 text-[#F97316] text-xs font-bold px-3 py-1.5 rounded-full mb-4">
              無料診断
            </div>
            <h2 className="text-2xl font-bold text-gray-900 mb-2">ベランダ劣化診断チェックリスト</h2>
            <p className="text-gray-600">当てはまる症状をチェックして、防水工事の必要性を確認しましょう</p>
          </div>
            ...
            {checklist.map((item, index) => {
              ...
              return (
                <div key={index} className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl border border-gray-100">
                  ...
                  <span className={`text-xs font-bold px-2 py-1 rounded-full ${config.color} flex-shrink-0`}>
                    {config.label}
                  </span>
                </div>
              );
          ...
          <div className="bg-[#EFF6FF] border border-[#2563EB]/20 rounded-xl p-6 text-center">
            <p className="text-sm text-gray-700 mb-4">
              <strong>「危険」「緊急」</strong>が1つでも当てはまる場合は早急な対応が必要です。
              無料見積もりで専門家に現地診断を依頼しましょう。
            </p>
            <Link
              href="/ranking/"
              className="inline-flex items-center gap-2 bg-[#F97316] hover:bg-orange-600 text-white font-bold px-6 py-3 rounded-full transition-colors no-underline"
            >
              <span className="text-xs bg-white text-[#F97316] px-1.5 py-0.5 rounded font-bold">PR</span>
              無料で業者を探す
            </Link>
      ...
      <section className="max-w-6xl mx-auto px-4 py-12">
        <div className="text-center mb-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">おすすめ業者 TOP3</h2>
          <p className="text-gray-600">費用・品質・サポートを総合評価した上位3社</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {TOP3.map((company, index) => {
            const rankColors = ["bg-yellow-400", "bg-gray-300", "bg-orange-400"];
            const rankBorders = ["border-yellow-300", "border-gray-200", "border-orange-200"];
            return (
              <div
                key={company.slug}
                className={`bg-white rounded-2xl border-2 ${rankBorders[index]} p-6 relative overflow-hidden`}
              >
                <div className={`absolute top-0 left-0 ${rankColors[index]} text-white text-xs font-black px-3 py-1 rounded-br-xl`}>
                  {index + 1}位
                </div>
                <div className="mt-4 mb-4">
                  <h3 className="font-bold text-gray-900 text-lg mb-2">{company.name}</h3>
                  <p className="text-sm text-[#2563EB] font-medium mb-3">{company.tagline}</p>
                ...
                <Link
                  ...
                  className="block w-full text-center bg-[#2563EB] hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition-colors no-underline text-sm"
                >
                  詳細・口コミを見る
                </Link>
              </div>
        ...
        <div className="text-center">
          <Link
            href="/ranking/"
            className="inline-flex items-center gap-2 text-[#2563EB] font-bold hover:underline no-underline"
          >
            全10社のランキングを見る →
          </Link>
        </div>
      ...
      <section className="bg-white py-12 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-2">工法別・面積別 費用早見表</h2>
            <p className="text-gray-600">2026年最新の費用相場（材工込み）</p>
          </div>
          <div className="overflow-x-auto rounded-xl border border-gray-200">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[#2563EB] text-white">
                  <th className="px-4 py-3 text-left font-bold">工法</th>
                  <th className="px-4 py-3 text-left font-bold">面積目安</th>
                  <th className="px-4 py-3 text-left font-bold">費用目安</th>
                  <th className="px-4 py-3 text-left font-bold">耐用年数</th>
                  <th className="px-4 py-3 text-left font-bold hidden sm:table-cell">おすすめ用途</th>
                </tr>
              </thead>
              <tbody>
                {costTable.map((row, index) => (
                  <tr key={index} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                    ...
                    <td className="px-4 py-3 font-bold text-[#059669]">{row.cost}</td>
                    <td className="px-4 py-3 text-gray-600">{row.life}</td>
                    <td className="px-4 py-3 text-gray-500 hidden sm:table-cell">{row.best}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="max-w-4xl mx-auto px-4 py-12">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">防水工事に関するよくある疑問にお答えします</p>
        </div>
