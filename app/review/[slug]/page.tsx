...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!r) return {};
  const url = `https://balcony-bousui-navi.com/review/${r.slug}/`;
  return {
    title: r.title,
    ...
    "@graph": [
      {
        ...
        headline: r.title,
        description: r.metaDescription,
        image: `https://balcony-bousui-navi.com${r.heroImage}`,
        dateModified: "2026-06-16",
        author: { "@type": "Organization", name: "ベランダ防水ナビ" },
        publisher: { "@type": "Organization", name: "ベランダ防水ナビ", url: "https://balcony-bousui-navi.com/" },
        mainEntityOfPage: `https://balcony-bousui-navi.com/review/${r.slug}/`,
      },
      {
        "@type": "FAQPage",
      ...
      <section id="reviews" className="scroll-mt-20 mb-12">
        ...
        <figure className="mt-8">
          <img src={r.subImage} alt={r.subAlt} width={1200} height={630}
            className="w-full rounded-2xl border border-border object-cover aspect-[1200/630]" />
          <figcaption className="mt-2 text-xs text-text-muted text-center">防水工法（FRP・ウレタン・シート）によって耐用年数や費用は異なります。</figcaption>
        </figure>
      </section>
